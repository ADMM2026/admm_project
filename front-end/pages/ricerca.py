"""
Pagina Turista — Ricerca attrazioni e alloggi.
"""
...
st.set_page_config(
    page_title="Piemonte Tourism — Ricerca",
    page_icon="P",
    layout="wide",
)
load_css()
...
# ── Filtri sidebar (dipendono dal tipo attivo) ─────────────────────────────────
...
try:
    if active_index == "attractions":
        hits, total = es_service.search_attractions(
            text=query,
            provinces=filters["provinces"],
            categories=filters["categories"],
        )
    else:
        hits, total = es_service.search_accommodations(
            text=query,
            provinces=filters["provinces"],
...
with c2:
    ...
    try:
        db_count = es_service.count_index(active_index)
    except Exception:
        db_count = "—"
    st.markdown(
        f"""<div class="metric-card">
