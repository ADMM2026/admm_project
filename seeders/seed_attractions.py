...
def csv_to_mongo_json(dir_attractions_paths, file_municipalities_path, dir_images_path):
    ...
    for file_name, config in SOURCES.items():
        ...
        for _, row in df.iterrows():
            ...
            if config['cat'] in ["Siti UNESCO", "Aree Storico Culturali"]:
                normalized_name = clean_string_for_image_name(name)
                potential_img = f"{normalized_name}.jpg"
                full_img_path = os.path.join(dir_images_path, potential_img)
                
                if os.path.exists(full_img_path):
                    image_filename = potential_img

            document_mongo = {
                "_id": f"ATT_{id_counter}",
                "category": config['cat'],
                "name": name,
...
def main(args):
    ...
    attractions = csv_to_mongo_json(
        ...
        dir_images_path=args.images_dir
    )

    collection.insert_many(attractions)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--images_dir", type=str,
                        default=os.path.join(script_dir, "img"),
